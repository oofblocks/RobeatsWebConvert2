...
module.export("osu_to_lua", function(osu_file_contents) {
	...
	append_to_output(format("Map.%s = \"%s\"","AudioAssetId","rbxassetid://FILL_IN_AUDIO_ASSETID_HERE"));
	//append_to_output("--The name of your map.")
	append_to_output(format("Map.%s = \"%s\"","Name",beatmap.Title));
	...
	append_to_output(format("Map.%s = \"%s\"","Background","rbxassetid://FILL_IN_COVERART_TEXTURE_ASSETID_HERE"));
	...
	append_to_output(`Map.BPM = ${beatmap.bpmMin == beatmap.bpmMax  && beatmap.bpmMin | `${beatmap.bpmMin} - ${beatmap.bpmMax}`}`)
	append_to_output(`Map.Tags = "${beatmap.Tags}"`)
	append_to_output(`Map.Mapper = "${beatmap.Creator}"`)
	...
	for (var i = 0; i < beatmap.hitObjects.length; i++) {
		...
		if (type == "slider") {
			append_to_output(format("Hold(%d,%d,%d) ", track, itr.startTime, itr.duration))
		} else {
			append_to_output(format("Note(%d,%d) ", track, itr.startTime))
		}
	}
	append_to_output("--")
